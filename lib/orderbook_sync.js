...
_.assign(OrderbookSync.prototype, new function() {
  ...
  prototype.loadOrderbook = function() {
    ...
    self.publicClient.getProductOrderBook(
      self.productID, bookLevel, function(err, response, body) {
      ...
      console.log('Loading new orderbook ' + data.sequence);
  ...
  prototype.processMessage = function(data) {
    ...
    if (self._sequence == -1) {
      ...
      console.log('resync in process ' + data.sequence);
      return;
    }
    ...
    if (data.sequence != self._sequence + 1) {
      ...
      console.log('RESYNC -> ' + data.sequence + ' : ' + self._sequence);
      self._queue = [];
      self._sequence = -1;
    ...
    console.log('Processing: ' + data.sequence);
