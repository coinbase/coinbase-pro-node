...
_.assign(Orderbook.prototype, new function() {
  ...
  prototype.getTree = function(side) {
  ...
  prototype.add = function(order) {
    ...
    var tree = self.getTree(order.side);
  ...
  prototype.match = function(match) {
    ...
    var tree = self.getTree(match.side);
    var node = tree.find({price: price});
    assert(node);

    var order = node.orders[0];
    assert(order.id == match.maker_order_id);
  ...
  prototype.change = function(change) {
    ...
    var tree = self.getTree(change.side);
    var node = tree.find({price: price});
    assert(node);
