 ...
 * @param {String[]} [subscribe] - The initial subscribe message. Default: {product_ids: ['BTC-USD'], channels: ['level2']}
  ...
  onOpen() {
    ...
    if (this.auth.secret) {
      let sig = signRequest(
        this.auth,
        'GET',
        subscribeMessage.channels ? '/users/self/verify' : '/users/self'
      );
