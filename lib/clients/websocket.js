...
class WebsocketClient extends EventEmitter {
  ...
  async throttle(){
    ...
    if(this.throttled_que.length > 0){

      let internet = await checkInternet('pro.coinbase.com' )
      let now = new Date().getTime()
      if(now > this.last_thrtl_call + this.throttle_delay && internet ){
        this.last_thrtl_call = now
        //call the function init_ws_stream
        await this.init_ws_stream()    
        this.throttled_que[0].resolve()          
        this.throttled_que = []
      }
