...
_.assign(AuthenticatedClient.prototype, new function() {
  ...
  prototype._placeOrder = function(params, callback) {
    ...
    if (!params.side) {
      throw "`opts` must include param `side`";
    }

    if (!params.product_id) {
      throw "`opts` must include param `product_id`";
    }

    if (params.type === 'limit') {
      if (!params.price) {
        throw "`opts` must include param `price`";
      }
      if (!params.size) {
        throw "`opts` must include param `size`";
      }
    }
    else if (params.type === 'market' || params.type === 'stop') {
      if (!params.size && !params.funds) {
        throw "`opts` must include either param `size` or `funds`";
      }
    }
    else {
        throw "invalid type";
